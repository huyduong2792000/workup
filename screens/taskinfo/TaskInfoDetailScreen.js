...
import DumbTaskInfoDetailScreen from 'components/taskinfo/DumbTaskInfoDetailScreen'
...
const TaskInfoDetailScreen = ({navigation,route}) => {
    ...
    navigation.setOptions({
        ...
        <View style={{paddingHorizontal:10, position:'relative'}}>
            <Text onPress={onDeleteTask} style={{fontSize:15,color:'red'}}>Xóa</Text>
        </View>
        ),
    }); 
    ...
    const getAvatar = (data) => {
        ...
        var name_display = name_test[0].slice(0,1) + name_test[name_test.length-1].slice(0,1)        
        return <Text style={{color:'white',position:'absolute'}}>{name_display}</Text>
    }
    useEffect(() => {
        if(id!=null){
            BaseApi({collectionName:'task_info'}).get({id:id}).then((response)=>{  
                // console.log(data);
                onSetCurrentTask(response.data)   
            })
        }else{
            const init_reducer = {assignee:currentUser,followers:[currentUser],status:0,note:'',task_code:null}
