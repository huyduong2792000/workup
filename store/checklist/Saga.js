...
import {setGroupSuggest} from 'store/group/Actions';
export function* getChecklist(action) {
  try {
    const response = yield BaseApi({collectionName: 'checklist'}).get({
      id: action.data,
    });
    yield put({type: SET_CHECKLIST, data: response.data});
  } catch (e) {
...
export function* saveChecklist(action) {
  try {
    if (action.data.checklist_name != '') {
      const data = yield BaseApi({collectionName: 'checklist'}).save(
        action.data,
      );
...
export function* addTaskInfo(action) {
  try {
    if (action.data.task_info_name === '') {
      DeviceEventEmitter.emit('showToast', {
        text: 'Tên việc không được để trống',
        type: 'warning',
      });
    } else {
      let task_info = yield BaseApi({collectionName: 'task_info'}).post(
        action.data,
      );
      yield put(setGroupSuggest(task_info.group));
      yield call(getChecklist, {data: action.data.checklist_id});
    }
  } catch (e) {
    DeviceEventEmitter.emit('showToast', {
      text: 'Đã có lỗi xảy ra khi tạo việc',
      type: 'danger',
    });
  }
}
